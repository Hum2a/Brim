...
The wrapper MUST fail loudly if no environment is given. `cf:sync` also goes through `with-env.mjs` and pushes Worker secrets to Cloudflare without printing values. Staging and production require `--yes`.
