...
describe("api", () => {
  ...
  it("searches the fixture VCA catalogue", async () => {
    ...
    expect(res.status).toBe(200);
    const json = (await res.json()) as { vehicles: Array<{ model: string; officialCycle: string }> };
  ...
  it("returns a catalogue row by id", async () => {
    const res = await app.request("/v1/vehicles/catalogue/vca_ford_focus_10_titanium", {}, { BRIM_FIXTURES: "1" });
    expect(res.status).toBe(200);
