...
| <kbd>npm run db:migrate:all</kbd> | drizzle migrate on dev, then staging, then production |
| <kbd>npm run cf:sync:staging</kbd> / <kbd>cf:sync:prod</kbd> | upload Worker secrets (`--yes`) |
| <kbd>npm run deploy:staging</kbd> | build SPA + API, deploy one Worker |
| <kbd>npm run deploy:prod</kbd> | same, production env |
| <kbd>npm run deploy:all</kbd> | staging Worker, then production Worker |
